...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Very light, Apple-style gradient - almost white
    float luminance = buf[0].x * 0.3 + buf[0].y * 0.5 + buf[0].z * 0.2;
    
    // Apple-inspired ultra-light palette
    vec3 pureWhite = vec3(1.0, 1.0, 1.0);
    vec3 softGray = vec3(0.98, 0.98, 0.99);
    vec3 warmTint = vec3(1.0, 0.995, 0.99);
    
    // Subtle gradient
    vec2 uv = vUv;
    float gradientMix = uv.y;
    
    vec3 baseGradient = mix(warmTint, softGray, gradientMix * 0.5);
    
    // Very subtle pattern overlay
    vec3 color = mix(baseGradient, pureWhite, luminance * 0.3);
    
    // Mix with opacity for blending
...
const CPPNShaderMaterial = shaderMaterial(
  { iTime: 0, iResolution: new THREE.Vector2(1, 1), uOpacity: 1 },
  vertexShader,
  fragmentShader
);
...
function ShaderPlane({ opacity = 1 }: { opacity?: number }) {
  ...
  return (
    <mesh ref={meshRef} position={[0, 0, 0]}>
      ...
      <cPPNShaderMaterial ref={materialRef} transparent />
    </mesh>
  );
}
...
export function ShaderBackground({ className = '', style, animate = true }: ShaderBackgroundProps) {
