...
const MobileSocialProof = ({ theme }: { theme: string | undefined }) => (
    <section id="hero" className="min-h-[90vh] w-full overflow-hidden bg-background pt-24 pb-0 relative flex flex-col justify-between">
        ...
        <div className="w-full relative z-30 flex flex-col items-center">
            <div
                className="text-center space-y-3 relative px-5"
                style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}
            >
                <div className="mb-4 inline-flex items-center gap-2 rounded-full backdrop-blur-xl bg-white/70 border border-border/30 px-3 py-1.5 shadow-sm">
                    <span className="rounded-full bg-primary px-2.5 py-1 text-[10px] font-bold text-primary-foreground tracking-wide">
                        SOHUB
                    </span>
                    ...
                    {[...initiatives, ...initiatives, ...initiatives, ...initiatives].map((item, index) => (
                        ...
                        >
                            <img
                                src={item.src}
                                alt={item.name}
                                className={`h-full w-auto object-contain text-foreground/50 ${item.name === 'PROTECT' ? 'mix-blend-multiply dark:mix-blend-normal' : ''}`}
                            />
                        </div>
                    ))}
                </InfiniteSlider>
            ...
            {/* CTA Buttons */}
            <div className="flex items-center justify-center gap-2.5 px-6 w-full mt-8 mb-4">
                <a
                    href="#initiatives"
                    className="group inline-flex items-center justify-center gap-1.5 rounded-full bg-[#F97316] px-6 py-2 text-[11px] font-semibold text-white shadow-lg transition-all duration-300 hover:bg-[#EA580C]"
                >
                    Initiatives
                ...
                <a
                    href="#why"
                    className="inline-flex items-center justify-center gap-2 rounded-full border border-border/50 bg-white px-4 py-2 text-[11px] font-medium text-foreground transition-all duration-300"
                >
                    Why we exist
                </a>
            </div>
        ...
        {/* Bottom Visual Area */}
        <div className="relative -mt-24 h-[320px] w-full overflow-hidden z-20 flex flex-col justify-end pb-28 [mask-image:linear-gradient(to_bottom,transparent,black_20%)]">
            ...
            <div className="relative z-30 w-full flex flex-col items-center gap-6">
                <div className="text-center">
                    <p className="mb-2.5 text-[10px] font-bold uppercase tracking-[0.2em] text-foreground/60">
                        Our Focus
                    </p>
                    <div className="flex flex-wrap items-center justify-center gap-2">
                        {['Discipline', 'Transparency', 'Results'].map((item) => (
                            <span
                                key={item}
                                className="rounded-full border border-border/30 bg-white px-3.5 py-1.5 text-[11px] font-medium text-foreground"
                            >
                                {item}
                            </span>
                        ))}
...
export const SocialProof = () => {
    ...
    if (isMobile) {
        return <MobileSocialProof theme={theme} />;
    }

    return <DesktopSocialProof theme={theme} />;
};
