        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto w-[95vw] sm:w-full bg-white border border-gray-200 shadow-2xl rounded-2xl">
          ...
          {!showForm ? (
            <div className="space-y-6 py-6">
              ...
              {selectedInterest && (
                <div className="bg-[#fff7e6] border border-[#fb8a09]/10 rounded-2xl p-6 space-y-4">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{interests[selectedInterest].emoji}</span>
                    <h3 className="text-[20px] font-medium text-[#202124]">{interests[selectedInterest].title}</h3>
                  </div>
                  <p className="text-[15px] text-[#3c4043] leading-relaxed">{interests[selectedInterest].description}</p>
                  {interests[selectedInterest].points && (
                    <div className="pt-2">
                      <p className="font-medium text-[#202124] mb-2 text-[15px]">We welcome those who understand:</p>
                      <ul className="list-none space-y-2 text-[15px] text-[#3c4043]">
